...
function checkAction(action, angles, positions, hands) {
  ...
  if (action === "spike") {
    ...
    if (angles.knee < 140) issues.push("knee_too_bent");
    if (angles.elbow < receiveLobsterElbowThreshold() && angles.shoulder < RECEIVE_LOBSTER_SHOULDER_RISK) {
      issues.push("lobster_receive_risk");
    }
...
const DEFAULT_BAND_TOLERANCE = 5;
const MAX_BAND_TOLERANCE = 24;
const RECEIVE_LOBSTER_ELBOW_RISK = 125;
const RECEIVE_LOBSTER_SHOULDER_RISK = 55;
const RECEIVE_LOBSTER_REFERENCE_MARGIN = 4;
...
function bandFor(action, entry, phase, joint) {
  ...
  if (referenceBand) {
    return { band: referenceBand, tolerance: referenceTolerance(referenceBand), source: "reference" };
  }
...
function referenceTolerance(band) {
  ...
  return Math.max(0, Math.min(MAX_BAND_TOLERANCE, tolerance));
}

function receiveLobsterElbowThreshold() {
  const bandInfo = bandFor("receive", referenceFor("receive"), "contact", "elbow");
  if (!bandInfo) return RECEIVE_LOBSTER_ELBOW_RISK;
  const acceptedLow = Math.max(0, bandInfo.band.p10 - bandInfo.tolerance);
  return Math.min(RECEIVE_LOBSTER_ELBOW_RISK, Math.max(0, acceptedLow - RECEIVE_LOBSTER_REFERENCE_MARGIN));
}
...
function evaluatePhaseJoint(action, entry, frames, phase, frameIndex, joint, rule, issues, issueFrames) {
  ...
  return {
    ...
    tolerance: Number(tolerance.toFixed(1)),
...
function evaluatePhaseHands(action, frames, contact, issues, issueFrames) {
  ...
  if (action === "receive") {
    ...
    if (
      (frame.angles.elbow || 180) < receiveLobsterElbowThreshold() &&
      (frame.angles.shoulder || 180) < RECEIVE_LOBSTER_SHOULDER_RISK
    ) {
